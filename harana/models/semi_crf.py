...
# The transition probability is disabled for now before a better formulation of the transition is proposed

class SemiCRF(nn.Module):

    def __init__(self, batch_size, num_label, segment_max_len, transition_importance, device):
        super().__init__()
        ...
        # the last number in the harmony label index list is BOS tag.
    ...
    def compute_path_score(self, harmony_index_seq):
        path_score = torch.zeros(self.batch_size)
        path_score = path_score.to(self.device)


        for batch_idx in range(self.batch_size):
            label_idx_cur = int(harmony_index_seq[batch_idx, 0].item())
            ...
            for frame_idx in range(self.sample_size):
                if harmony_index_seq[batch_idx, frame_idx] != label_idx_cur:
                    end_frame = frame_idx - 1
                    label_idx_new = int(harmony_index_seq[batch_idx, frame_idx].item())
                    #transition_score_cur += self.transitions[label_idx_cur, label_idx_cur] * (end_frame - start_frame) + self.transitions[label_idx_cur, label_idx_new]
                    ...
                    path_score[batch_idx] += + segment_score_cur
                    transition_score_cur = 0
        ...
        for end_frame in range(1, self.sample_size):
            for label_cur_idx in range(self.num_label):
                ...
                if end_frame + 1 <= self.segment_max_len:
                    ...
                    complete_score_cur_starting_at_0 = segment_score_cur_starting_at_0.unsqueeze(-1)
                
                # Segments not starting at 0
                # j + 1 is the length of the frame
                segment_score_cur_with_harmony_change = torch.stack([self.segment_score[:, end_frame - max_span + 2 + j, end_frame, label_cur_idx] * (j + 1) for j in range(max_span - 1)], dim=1)
                #transition_score_cur_with_harmony_change = self.transitions[torch.tensor(range(self.num_label)), label_cur_idx] * self.transition_importance
                #transition_score_cur_with_harmony_change = torch.stack([transition_score_cur_with_harmony_change + self.transitions[label_cur_idx, label_cur_idx] * self.transition_importance * j for j in range(max_span - 1)], dim=0)
                #complete_score_cur_with_harmony_change = segment_score_cur_with_harmony_change[:, :, None] + transition_score_cur_with_harmony_change[None, :, :]
                complete_score_cur_with_harmony_change = segment_score_cur_with_harmony_change[:, :, None]
                
                log_alpha_pre_with_harmony_change = log_alpha[:, end_frame - max_span + 1 : end_frame, :]

                if end_frame + 1 <= self.segment_max_len:
                    log_alpha[:, end_frame, label_cur_idx] = torch.cat([complete_score_cur_starting_at_0, (log_alpha_pre_with_harmony_change + complete_score_cur_with_harmony_change).flatten(start_dim=1)], dim=1).logsumexp(dim=1)
                else:
                    log_alpha[:, end_frame, label_cur_idx] = (log_alpha_pre_with_harmony_change + complete_score_cur_with_harmony_change).flatten(start_dim=1).logsumexp(dim=1)
            ...
            for end_frame in range(1, self.sample_size):
                for label_cur_idx in range(self.num_label):
                    ...
                    if end_frame + 1 <= self.segment_max_len:
                        ...
                        complete_score_cur_starting_at_0 = segment_score_cur_starting_at_0
                    
                    # Segments not starting at 0
                    # j + 1 is the length of the frame
                    segment_score_cur_with_harmony_change = torch.stack([self.segment_score[sample_idx, end_frame - max_span + 2 + j, end_frame, label_cur_idx] * (j + 1) for j in range(max_span - 1)], dim=0)
                    #transition_score_cur_with_harmony_change = self.transitions[torch.tensor(range(self.num_label)), label_cur_idx] * self.transition_importance
                    #transition_score_cur_with_harmony_change = torch.stack([transition_score_cur_with_harmony_change + self.transitions[label_cur_idx, label_cur_idx] * self.transition_importance * j for j in range(max_span - 1)], dim=0)
                    #complete_score_cur_with_harmony_change = segment_score_cur_with_harmony_change[:, None] + transition_score_cur_with_harmony_change
                    complete_score_cur_with_harmony_change = segment_score_cur_with_harmony_change[:, None]
                    
                    V_pre = V[end_frame - max_span + 1 : end_frame, :]
                    
                    total_score_with_harmony_change = V_pre + complete_score_cur_with_harmony_change
                    
                    pre_pos_with_harmony_change = (total_score_with_harmony_change==torch.max(total_score_with_harmony_change)).nonzero()
                    pre_pos_with_harmony_change = pre_pos_with_harmony_change[0, :]
                    total_score_with_harmony_change_max = total_score_with_harmony_change[pre_pos_with_harmony_change[0], pre_pos_with_harmony_change[1]]
                    
                    if total_score_with_harmony_change_max > complete_score_cur_starting_at_0:
                        V[end_frame, label_cur_idx] = total_score_with_harmony_change_max
                        pre_pos_with_harmony_change[0] += end_frame - max_span + 1
                        pre_pos[end_frame, label_cur_idx] = pre_pos_with_harmony_change
                    else:
