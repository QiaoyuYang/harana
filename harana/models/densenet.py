...
class DenseNet(nn.Module):
    def __init__(self, in_channels=12, block_depths=[2, 2, 2], growth_rates=[16, 8, 4], kernel_sizes=[3, 5, 7], bottleneck=True, drop_rate=0.6):
        super().__init__()
...
class DensePoolNet(nn.Module):
    
    def __init__(self, in_channels=12, block_depths=[3, 3], growth_rates=[8, 4], kernel_sizes=[3, 5], bottleneck=True, drop_rate=0.6):
    ...
    def forward(self, x):
        ...
        out = self.maxpool1(self.block1(x))
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.lkrelu(self.bn(self.conv(x)))
...
class BottleneckBlock(nn.Module):
    ...
    def forward(self, x):
        out = self.lkrelu(self.bn1(self.conv1(x)))
        out = self.lkrelu(self.bn2(self.conv2(out)))
